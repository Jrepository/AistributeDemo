		...
		boolean canDoTask(TaskInfo taskInfo) {
			...
			if(this.doneTasks.size()<2) {
				...
				//}
					...
					remark="我在跨码头 已经超过2次，不能再接码头："+taskInfo.portCode;
				}
				
		    }
			else if(this.doneTasks.size()<5) {
		    	String samePort=donePortSet.iterator().next();
		    	if(this.doneTasks.size()>1) {
		    		re=false;
		    		remark="我在跨码头 "+samePort+" 已经达3次，不能再去其他码头："+taskInfo.portCode;
		    	}
		    	if(taskInfo.portCode.equals(samePort)) {
					...
					re=false;
					remark="我在同一个码头 "+samePort+" 已经超过4次，不能再去其他码头："+taskInfo.portCode;
				}
		    }
